...
type Settings struct {
	...
	// MaxTimeout is the maximum timeout used for open-state recovery delay.
	// If MaxTimeout is 0, open-state timeout is fixed at Timeout.
	// If MaxTimeout is greater than 0, each failed half-open probe doubles Timeout
	// up to MaxTimeout.
...
func NewCircuitBreaker[T any](st Settings) *CircuitBreaker[T] {
	...
	cb.baseTimeout = cb.timeout
	cb.maxTimeout = 0
	if st.MaxTimeout > 0 {
		if st.MaxTimeout < cb.baseTimeout {
			cb.maxTimeout = cb.baseTimeout
		} else {
...
func (cb *CircuitBreaker[T]) setState(state State, now time.Time) {
	...
	prev := cb.state
	if cb.maxTimeout > 0 && prev == StateHalfOpen && state == StateOpen {
		cb.timeout *= 2
		if cb.timeout > cb.maxTimeout {
			cb.timeout = cb.maxTimeout
		}
	}
	if cb.maxTimeout > 0 && prev == StateHalfOpen && state == StateClosed {
